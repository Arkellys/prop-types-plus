...
import { requiredIf } from "../src/index.js";
import validate, { PROP_NAME } from "./_validate.js";


describe("requiredIf", () => {
	it("throws an error when the input condition is not a function", () => {
		expect(() => requiredIf(PropTypes.string)).to.throw(TypeError);
		expect(() => requiredIf(PropTypes.string, null)).to.throw(TypeError);
		expect(() => requiredIf(PropTypes.string, {})).to.throw(TypeError);
		expect(() => requiredIf(PropTypes.string, "")).to.throw(TypeError);
		expect(() => requiredIf(PropTypes.string, 8015)).to.throw(TypeError);
	});

	it("doesn't throw an error when the input condition is a function", () => {
		expect(() => requiredIf(PropTypes.string, Function.prototype)).not.to.throw(TypeError);
	});

	it("fails when the validator is required", () => {
		const validator = requiredIf(PropTypes.string, Function.prototype).isRequired;
	...
	it("fails when the prop is not of the expected type", () => {
		const validator = requiredIf(PropTypes.string, Function.prototype);
