...
import { unnecessaryWhen } from "../src/index.js";
...
describe("unnecessaryWhen", () => {
	it("throws an error when the input condition is not a function", () => {
		expect(() => unnecessaryWhen(PropTypes.string)).to.throw(TypeError);
		expect(() => unnecessaryWhen(PropTypes.string, null)).to.throw(TypeError);
		expect(() => unnecessaryWhen(PropTypes.string, {})).to.throw(TypeError);
		expect(() => unnecessaryWhen(PropTypes.string, "")).to.throw(TypeError);
		expect(() => unnecessaryWhen(PropTypes.string, 8015)).to.throw(TypeError);
	});

	it("doesn't throw an error when the input condition is a function", () => {
		expect(() => unnecessaryWhen(PropTypes.string, Function.prototype)).not.to.throw(TypeError);
	});

	it("fails when the validator is required", () => {
		const validator = unnecessaryWhen(PropTypes.string, Function.prototype).isRequired;
	...
	it("fails when the prop is not of the expected type", () => {
		const validator = unnecessaryWhen(PropTypes.string, Function.prototype);
	...
	it("fails with a custom message when condition is not satisfied", () => {
		...
		const validator = unnecessaryWhen(
			PropTypes.string,
			(props) => props[anotherPropName],
			customMessage
		);
